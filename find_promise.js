...
// Creer un prompt qui prend le "firstName" et "lastName"
...
var findRecursivityPeople = function(firstName, lastName, tab, path) {
    for (var i=0; i < tab.length; i++) {
        ...
        if (element.children)
        var value = findRecursivityPeople(firstName, lastName, element.children, path+element.firstName+" ",+element.lastName+"/")
        if (value)
            return value
